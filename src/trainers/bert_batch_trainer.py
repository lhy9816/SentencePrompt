"""
The trainer for our duplicate batching scheme
"""
...
from transformers.optimization import get_scheduler, get_polynomial_decay_schedule_with_warmup
...
class BertTrainer(Trainer):
    ...
    def __init__(self, raw_dataset, model_args, data_args, eval_data_collator, *args, **kwargs):
        ...
        # Merge all datasets together, in our case there is only training set
        self.merged_dataset = concatenate_datasets([raw_dataset[split] for split in raw_dataset.keys()])
        # Add dataset_to_sent_id to map input sentence to its sent_id and use that 
        # to obtain its sentence vector
        self.dataset_to_sent_id = dict((sent, sent_id) for sent, sent_id in \
            zip(self.merged_dataset['text'], self.merged_dataset['sentences_ids']))
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        Here we add OurReduceLROnPlateau, OurExponentialLR, our_polynomial_decay_scheduler, and multistep scheduler.
        Args:
            num_training_steps (int): The number of training steps to do.
            optimizer (torch.optim.Optimizer): The optimizer we use
        """
    ...
    def evaluate(
        ...
        if self.state.epoch % self.data_args.eval_every_k_epochs == 0 and self.state.epoch >= self.data_args.eval_after_k_epochs:
            ...
            # Add previous best metric if we don't compute the metric in this epoch, Modified!
            dummy_metrics = {}
    ...
    def prediction_step(
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    ...
                    with self.autocast_smart_context_manager():
                        # Here the compute loss should return element-wise loss, one loss for each instance in each batch
        ...
        # The last item in logits is the new labels if its length is 2, Modified!
    ...
    def evaluate_senteval(self) -> Dict[str, float]:
        ...
        def batcher(params, batch):
            ...
            # Obtain the sentence to id dict  so that we can find its sentence.
        ...
        def batcher_mix(params, batch, evaluate_mode='avg'):
            """
            Overwrite SentEval batcher methods
            """
            sentences = [' '.join(s) for s in batch]
            ...
            with torch.no_grad():
                ...
                if self.model_args.plm_name == 'bert' or '-bert' in self.model_args.plm_name:
                    pretrained_model = self.model.bert
                    token_type_ids = batch_input['token_type_ids']
                elif self.model_args.plm_name == 'roberta' or '-roberta' in self.model_args.plm_name:
        ...
        def batcher_concate(params, batch, evaluate_mode='avg'):
            sentences = [' '.join(s) for s in batch]
        ...
        # Set params for SentEval, we only use Fast eval during training
        # Fast eval
        ...
        # Only evaluating pure sent embedding duing training
    ...
    def evaluate_senteval_final(self) -> Dict[str, float]:
        """
        Evaluate trained sentence prompts on senteval tasks.
        """
        # memory metrics - must set up as early as possible
        self._memory_tracker.start()

        # Add final evaluate mode attribute, decide the concatenated plm embedding
        ...
        def batcher_concate(params, batch):
            ...
            with torch.no_grad():
                # Obtain sentence prompts from the trained sentence vectors
                sent_ids = torch.tensor([sent_to_id[sent.strip()] for sent in sentences]).to(self.args.device)
                sent_vec_outputs = self.model.sentence_embedding.sentence_embedding(sent_ids)
                # Obtain sentence embeddings by passing through the bert
                if self.model_args.plm_name == 'bert' or '-bert' in self.model_args.plm_name:
        ...
        # Post processing
        for task in self.target_tasks:
            if task in ['WILA', 'WILB', 'WILC', 'MR', 'CR', 'SUBJ', 'MPQA', 'SST2', 'SST5', 'TREC', 'MRPC', 'Length', 'WordContent', \
                        'Depth', 'TopConstituents','BigramShift', 'Tense', \
