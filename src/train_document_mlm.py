...
"""
Main entrance to train doc-level tasks in duplicate batch scheme
...
import pdb
from typing import Optional
from trainers.bert_document_trainer import BertTrainer
...
from utils.data_collators import DocumentDataCollatorForLanguageModeling, MyDataCollatorForLanguageModeling, MyDataCollatorForSpanMask, MyDataCollatorForWholeWordMask, BatchDataCollatorForSpanMask
...
class ModelArguments:
    ...
    mlm_loss_no_reduction: bool = field (
        default=False,
        metadata={
            "help": "Whether use reduction='mean' to compute the mlm loss."
        }
    )
...
class DataTrainingArguments:
    ...
    select_span_length: Optional[int] = field(
        default=None,
        metadata={"help": "The span length to be selected of one instance in one batch."}
    )
...
def main():
    ...
    if data_args.line_by_line:
        ...
        raise NotImplementedError("Can only support processing one document in one line!")
    # else:
    ...
    if model_args.lm_head_name == 'random_mask':
        data_collator = DocumentDataCollatorForLanguageModeling(
            tokenizer=tokenizer,
            mlm_probability=data_args.mlm_probability,
            pad_to_multiple_of=8 if pad_to_multiple_of_8 else None,
            duplicate_times=data_args.duplicate_times,
            span_length=data_args.select_span_length
        )
        eval_data_collator = DocumentDataCollatorForLanguageModeling(
            tokenizer=tokenizer,
            mlm_probability=data_args.mlm_probability,
            pad_to_multiple_of=8 if pad_to_multiple_of_8 else None,
            duplicate_times=1,
    ...
    init_sent_vector_embedding(model, model_args, tokenizer, training_args, train_dataset)
    ...
    # Set stop training argument to 50 for BERT and 70 for RoBERTa
    stop_training_callback = StopTrainingCallback(data_args.early_stopping_patience)
    ...
    if training_args.do_eval and training_args.save_strategy.value != 'no':
        ...
        metrics = {}
        # Separately save document vector
    ...
    write_final_results_to_one_file(model_args, data_args, training_args, sent_eval_metrics, save_file_path='../final_doc_result.csv')
