...
PLM_NAME=bert
TASK=CR           # When TASK is SICK related tasks, set the datadir to SICK
BATCH_SIZE=1
LR=5e-1                           # 3e-5
PROMPT_LENGTH=1              # 2
NUM_TRAIN_EPOCHS=1
...
python train_batch_mlm.py \
    --model_name_or_path ../pretrained_models/$PLM_NAME-base \
    --train_file ../dataset/$TASK/raw_data.txt \
    --cache_dir ../cache_dir \
    --output_dir ../checkpoints/$PLM_NAME-base\_$TASK/batch-eval-optimizer-$OPTIMIZER_NAME-seed-$SEED-my-mlm_prob-$MLM_PROB-lr-$LR-duptime-$DUPLICATE_TIMES-mask_scheme-$MASK_SCHEME-lossacc-trainepoch-$NUM_TRAIN_EPOCHS-prompt_length-$PROMPT_LENGTH-scheduler-$LR_SCHEDULER-warmmup-$WARMUP_RATIO-weightdecay-$WEIGHT_DECAY-batch_size-$BATCH_SIZE \
    ...
    # --resume_from_checkpoint ../checkpoints/$PLM_NAME-base\_$TASK/batch-eval-optimizer-$OPTIMIZER_NAME-seed-$SEED-my-mlm_prob-$MLM_PROB-lr-$LR-duptime-$DUPLICATE_TIMES-mask_scheme-$MASK_SCHEME-lossacc-trainepoch-$NUM_TRAIN_EPOCHS-prompt_length-$PROMPT_LENGTH-scheduler-$LR_SCHEDULER-warmmup-$WARMUP_RATIO-weightdecay-$WEIGHT_DECAY-batch_size-$BATCH_SIZE/checkpoint-746340
    # str, The SentEval task to be selected to train / evaluate.
