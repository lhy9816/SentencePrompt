...
from senteval.ged import GEDEval, GEDEvalErrorType
from senteval.document import DocumentEval, DocumentInnerKfoldEval
...
class SE(object):
    def __init__(self, params, batcher, prepare=None):
        ...
        self.list_tasks = ['IMDB', 'HyperParNews',
                           'WILA', 'WILB', 'WILC',
    ...
    def eval(self, name):
        ...
        # Grammar Error Detection tasks
        elif name == 'WILA':
            self.evaluation = GEDEvalErrorType(tpath + '/downstream/WIL/A', seed=self.params.seed)
        elif name == 'WILB':
            self.evaluation = GEDEvalErrorType(tpath + '/downstream/WIL/B', seed=self.params.seed)
        elif name == 'WILC':
            self.evaluation = GEDEvalErrorType(tpath + '/downstream/WIL/C', seed=self.params.seed)

        # Document Classification tasks
        elif name == 'IMDB':
            self.evaluation = DocumentEval(tpath + '/downstream/IMDB', seed=self.params.seed)
        elif name == 'HyperParNews':
            self.evaluation = DocumentInnerKfoldEval(tpath + '/downstream/HyperParNews', seed=self.params.seed)
            # self.evaluation = DocumentEval(tpath + '/downstream/HyperParNews', seed=self.params.seed)
